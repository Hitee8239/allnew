...
myidx = ['강감찬','김유신','이순신','안익태','윤동주']
member = Series(data=[30,20,40,30,60],index=myidx)

member.plot(kind='bar',rot=0,ylim=[0,member.max()+5],use_index=True, grid=False,table=False, color=['r','g','b','y','m'])
plt.xlabel('학생 이름')
plt.ylabel('점수')
plt.title('학생별 시험 점수')
