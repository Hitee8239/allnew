...
app.get('/hello', (req, res) => {
    urls = "http://3.37.199.29:8000/hello";
...
app.get("/api/users", (req, res) => {
    axios
        .get('http://3.37.199.29:8000/api/users')
        .then(result => {
...
app.get("/api/users/user", (req, res) => {
    if (req.query.name == null) {
        urls = "http://3.37.199.29:8000/api/users/user?user_id=" + req.query.user_id;
    } else {
        urls = "http://3.37.199.29:8000/api/users/user?user_id=" + req.query.user_id + "&name=" + req.query.name;
    }
    request(urls, { json: true }, (err, result, body) => {
        if (err) { return console.log(err); }
        res.send(CircularJSON.stringify(body))
...
app.get("/api/users/:user_id", (req, res) => {
    urls = "http://3.37.199.29:8000/api/users/" + req.params.user_id;
    request(urls, { json: true }, (err, result, body) => {
        if (err) { return console.log(err); }
        res.send(CircularJSON.stringify(body))
    })
...
app.post("/api/users/userBody", (req, res) => {
    const option = {
        uri: 'http://3.37.199.29:8000/api/users/userBody',
        method: 'POST',
        form: { id: req.body.id }
    }
...
app.post("/api/users/add", (req, res) => {
    const option = {
        uri: 'http://3.37.199.29:8000/api/users/add',
        method: 'POST',
        form: {
            id: req.body.id,
            name: req.body.name
...
app.put("/api/users/update", (req, res) => {
    const option = {
        uri: 'http://3.37.199.29:8000/api/users/update',
        method: 'PUT',
        form: {
            id: req.body.id,
            name: req.body.name
...
app.patch("/api/users/update/:user_id", (req, res) => {
    const option = {
        uri: 'http://3.37.199.29:8000/api/users/update/' + req.params.user_id,
        method: 'PATCH',
        form: {
            name: req.body.name
        }
...
app.delete("/api/users/delete", (req, res) => {
    const option = {
        uri: 'http://3.37.199.29:8000/api/users/delete/',
        method: 'DELETE',
        form: {
